...
def create_kind_cluster(cluster_name: str) -> bool:
    ...
    try:
        ...
        subprocess.run(["kind", "create", "cluster", "--name", cluster_name], check=True, capture_output=True, text=True)
        return True
    except subprocess.CalledProcessError as e:
        # You could log the error here for debugging
...
def get_kind_kubeconfig(cluster_name: str) -> str | None:
    ...
    """
    try:
        result = subprocess.run(["kind", "get", "kubeconfig", "--name", cluster_name], check=True, capture_output=True, text=True)
...
def delete_kind_cluster(cluster_name: str) -> bool:
    ...
    """
    try:
        subprocess.run(["kind", "delete", "cluster", "--name", cluster_name], check=True, capture_output=True, text=True)
        return True
    except subprocess.CalledProcessError as e:
