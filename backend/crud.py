...
def get_cluster_by_name(db: Session, user_id: str, name: str):
    ...
    return db.query(models.Cluster).filter(
        models.Cluster.name == name, models.Cluster.user_id == user_id
    ).first()
...
def get_clusters_by_user(db: Session, user_id: str):
    ...
    user_team_ids = db.query(models.team_memberships.c.team_id).filter(models.team_memberships.c.user_id == user_id).subquery()
...
def get_expired_clusters(db: Session):
    ...
    return db.query(models.Cluster).filter(
        models.Cluster.ttl_expires_at <= datetime.now(timezone.utc)
    ).all()
...
def get_team(db: Session, team_id: str):
    ...
    return db.query(models.Team).options(joinedload(models.Team.members)).filter(models.Team.id == team_id).first()

def create_team(db: Session, team: schemas.team.TeamCreate, owner: models.User):
...
def get_teams_for_user(db: Session, user_id: str):
    ...
    return db.query(models.Team).join(models.team_memberships).filter(models.team_memberships.c.user_id == user_id).all()

def add_user_to_team(db: Session, team: models.Team, user: models.User, role: str = "member"):
