...
# --- END: ADD THE MISSING FUNCTION HERE ---
...
async def run_cluster_deletion(cluster_name: str, cluster_id: str, user_id: str):
    ...
    try:
        ...
        await asyncio.to_thread(
            provisioner.delete_cluster, cluster_name, db_cluster.provider
        )
...
async def delete_cluster(
    ...
    background_tasks.add_task(
        run_cluster_deletion, db_cluster.name, db_cluster.id, str(current_user.id)
    )
...
@teams_router.get(
    "/{team_id}", response_model=TeamDetails, dependencies=[Depends(require_member)]
)
