...
def load_config() -> dict:
    if not CONFIG_FILE.exists():
        CONSOLE.print(
            f"[bold red]Error:[/bold red] Not logged in. Please run '[cyan]pyk8s auth login[/cyan]' first."
        )
        sys.exit(1)
    try:
...
def get_auth_headers() -> dict:
    ...
    if not token:
        CONSOLE.print(
            f"[bold red]Error:[/bold red] Access token not found. Please run '[cyan]pyk8s auth login[/cyan]' again."
        )
        sys.exit(1)
...
def login():
    ...
    try:
        with CONSOLE.status(
            "[bold green]Authenticating...[/bold green]", spinner="dots"
        ):
        ...
        CONSOLE.print(
            f"✅ [bold green]Login successful![/bold green] Configuration saved to [cyan]{CONFIG_FILE}[/cyan]."
        )
    except httpx.HTTPStatusError as e:
        CONSOLE.print(
            f"[bold red]Error:[/bold red] API returned status {e.response.status_code}: {e.response.json().get('detail', e.response.text)}"
        )
    except httpx.RequestError as e:
        CONSOLE.print(
            f"[bold red]Connection or Timeout Error:[/bold red] Could not communicate with the API at [cyan]{API_BASE_URL}[/cyan]. ({e})"
        )
...
@click.option(
    "--ttl-hours", default=1, type=int, help="Time-to-live for the cluster in hours."
)
def create_cluster(name: str, ttl_hours: int):
    ...
    try:
        ...
        with CONSOLE.status(
            f"[bold green]Sending request to create cluster '{name}'...[/bold green]"
        ):
            with httpx.Client(timeout=TIMEOUT) as client:
                response = client.post(
                    f"{API_BASE_URL}/clusters", headers=headers, json=payload
                )
                response.raise_for_status()
        CONSOLE.print(
            f"✅ [bold green]Request accepted![/bold green] Cluster '{name}' is now provisioning."
        )
        ...
        CONSOLE.print(
            f"[bold red]Connection or Timeout Error:[/bold red] Could not communicate with the API. ({e})"
        )
...
def delete_cluster(name: str):
    ...
    try:
        ...
        with httpx.Client(timeout=TIMEOUT) as client:
            with CONSOLE.status(
                f"[bold yellow]Finding cluster '{name}'...[/bold yellow]"
            ):
            ...
            cluster_to_delete = next(
                (c for c in clusters_data if c["name"] == name), None
            )
            if not cluster_to_delete:
                CONSOLE.print(
                    f"[bold red]Error:[/bold red] Cluster '{name}' not found."
                )
                return
            cluster_id = cluster_to_delete["id"]
            with CONSOLE.status(
                f"[bold red]Sending request to delete cluster '{name}'...[/bold red]"
            ):
                delete_response = client.delete(
                    f"{API_BASE_URL}/clusters/{cluster_id}", headers=headers
                )
                delete_response.raise_for_status()
        CONSOLE.print(
            f"✅ [bold green]Request accepted![/bold green] Cluster '{name}' is scheduled for deletion."
        )
    except httpx.HTTPStatusError as e:
        CONSOLE.print(
            f"[bold red]Error:[/bold red] API returned status {e.response.status_code}: {e.response.json().get('detail', e.response.text)}"
    ...
    try:
        ...
        with httpx.Client(timeout=TIMEOUT) as client:
            ...
            if cluster_to_get["status"] != "RUNNING":
                CONSOLE.print(
                    f"[bold red]Error:[/bold red] Kubeconfig is only available for clusters in 'RUNNING' status. Status is currently '{cluster_to_get['status']}'."
                )
                sys.exit(1)
            ...
            with CONSOLE.status(
                f"[bold green]Fetching kubeconfig for '{name}'...[/bold green]"
            ):
                kubeconfig_response = client.get(
                    f"{API_BASE_URL}/clusters/{cluster_id}/kubeconfig", headers=headers
                )
