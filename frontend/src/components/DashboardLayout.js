                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
                    <div className="flex items-center space-x-8">
                        ...
                        <nav className="flex space-x-4">
                            <Link href="/dashboard" className={navLinkClasses('/dashboard')}>Clusters</Link>
                            <Link href="/dashboard/teams" className={navLinkClasses('/dashboard/teams')}>Teams</Link>
                        </nav>
                    </div>
                    <div className="flex items-center space-x-4">
                        <span className="text-gray-600 dark:text-gray-300 hidden sm:inline">{user?.email}</span>
                        <button onClick={handleLogout} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium transition-colors">Logout</button>
                    </div>
                </div>
            </header>
