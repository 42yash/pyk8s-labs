...
import Terminal from "@/components/Terminal"; // Make sure Terminal component is imported
import {
...
export default function Dashboard() {
	const { data: clusters, error: clustersError, isLoading: clustersLoading, isFetching: clustersFetching } = useGetClustersQuery();
	...
	const [showTerminalFor, setShowTerminalFor] = useState(null); // Will hold the cluster ID
	...
	return (
		<DashboardLayout>
			...
			{showTerminalFor && (
				<Terminal
					clusterId={showTerminalFor}
					onClose={() => setShowTerminalFor(null)}
				/>
			)}
			...
			<div className="space-y-8">
				...
				<div>
					...
					{(clustersLoading || clustersFetching) && <p>Loading clusters...</p>}
					{clustersError && <p className="text-red-500">Error fetching clusters: {clustersError.toString()}</p>}
								...
								{clusters?.map((cluster) => (
									<li key={cluster.id} className="p-4 flex justify-between items-center">
										<div>
											...
											<p className="text-sm text-gray-500">Provider: {cluster.provider}</p>
											<p className="text-sm text-gray-500">Expires: {new Date(cluster.ttl_expires_at).toLocaleString()}</p>
										</div>
										{/* --- THIS BUTTON GROUP WAS MISSING THE TERMINAL BUTTON --- */}
										<div className="flex items-center space-x-2">
											{renderStatusBadge(cluster.status)}
											<button
												onClick={() => setShowTerminalFor(cluster.id)}
												disabled={cluster.status !== 'RUNNING'}
												className="px-4 py-1 text-sm bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
											>
												Terminal
											</button>
											<button
